...
class AttachedText extends PsychAlphabet
{
...
class PsychAlphabet extends FlxSpriteGroup
{
	...
	var lastSprite:PsychAlphaCharacter;
	var xPosResetted:Bool = false;

	var splitWords:Array<String> = [];

	public var isBold:Bool = false;
	public var lettersArray:Array<PsychAlphaCharacter> = [];
	...
	public function addText()
	{
		...
		for (character in splitWords)
		{
			...
			var isNumber:Bool = PsychAlphaCharacter.numbers.indexOf(character) != -1;
			var isSymbol:Bool = PsychAlphaCharacter.symbols.indexOf(character) != -1;
			var isAlphabet:Bool = PsychAlphaCharacter.alphabet.indexOf(character.toLowerCase()) != -1;
			if ((isAlphabet || isSymbol || isNumber) && (!isBold || !spaceChar))
			{
				...
				// var letter:PsychAlphaCharacter = new PsychAlphaCharacter(30 * loopNum, 0, textSize);
				var letter:PsychAlphaCharacter = new PsychAlphaCharacter(xPos, 0, textSize);
	...
	public function timerCheck(?tmr:FlxTimer = null) {
		...
		if(loopNum <= splitWords.length && splitWords[loopNum] != null) {
			...
			var isNumber:Bool = PsychAlphaCharacter.numbers.indexOf(splitWords[loopNum]) != -1;
			var isSymbol:Bool = PsychAlphaCharacter.symbols.indexOf(splitWords[loopNum]) != -1;
			var isAlphabet:Bool = PsychAlphaCharacter.alphabet.indexOf(splitWords[loopNum].toLowerCase()) != -1;
			...
			if ((isAlphabet || isSymbol || isNumber) && (!isBold || !spaceChar))
			{
				...
				// var letter:PsychAlphaCharacter = new PsychAlphaCharacter(30 * loopNum, 0, textSize);
				var letter:PsychAlphaCharacter = new PsychAlphaCharacter(xPos, 55 * yMulti, textSize);
				letter.row = curRow;
				if (isBold)
				{
					if (isNumber)
...
class PsychAlphaCharacter extends FlxSprite
{
