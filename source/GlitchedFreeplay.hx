...
class GlitchedFreeplay extends MusicBeatState
{
	var songs:Array<SongMetadataaa> = [];
	...
	{
		var initSonglist = CoolUtil.coolTextFile(Paths.txt('glitchedState'));

		for (i in 0...initSonglist.length)
		{
			var data:Array<String> = initSonglist[i].split(':');
			songs.push(new SongMetadataaa(data[0], Std.parseInt(data[2]), data[1], Std.parseInt(data[3])));
	...
	{
		songs.push(new SongMetadataaa(songName, weekNum, songCharacter, colorTypeInt));
	}
